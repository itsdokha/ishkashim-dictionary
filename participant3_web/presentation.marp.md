...
<div class="team">
<div class="person"><div class="emoji">📚</div><div class="role">Данные и словарные статьи</div><div class="name">Султонов Диер</div></div>
...
<small>базовая лексика с полной разметкой</small>
...
## 📚 Задачи · Часть 1 — данные и словарные статьи

<span class="author">👤 Султонов Диер</span>

- Разработали единый **формат словарной статьи** — по нему оформлены все слова.
...
- Данные генерируются скриптом и проверяются валидатором.
...
- **Нормализация**: нижний регистр, снятие диакритики, `ё → е`.
...
- **Строка поиска** с мгновенной выдачей результатов по мере ввода.
- **Режимы поиска**: по ишкашимскому, по русскому или везде; фильтр по части речи.
- **Список результатов** с подсветкой совпадений.
- **Карточка слова**: перевод, варианты, примеры, этимология, источники.
- **Адаптивный дизайн** — удобно и на компьютере, и на телефоне.
...
- Источники старые, транскрипции в них расходятся — унифицировали вручную.
- У языка нет письменности → выбрали латиницу без диакритики как главный ключ, плюс кириллицу и варианты написания.
- Многозначность и омонимы (например, `wak` «один» и `wak` «там») — заложили как отдельные статьи.
